...
df_rul_FD4 = pd.read_excel("C:/Users/raviv/.spyder-py3/CMAPSSData/RUL-04.xlsx")

# Drop NaN columns
df_train_FD4.dropna(axis=1, inplace=True)
df_test_FD4.dropna(axis=1, inplace=True)
...
max_cycles_test = df_test_FD4.groupby('Unit No.')['time, in cycles'].transform('max')
max_cycles_per_unit= max_cycles_test.to_frame()
max_cycles_per_unit["Unit No."]=df_test_FD4["Unit No."]
df_FD4_merged = max_cycles_per_unit.merge(df_rul_FD4, on='Unit No.', how='left')
df_FD4_merged['Actual Max Cycles'] = df_FD4_merged['time, in cycles'] + df_FD4_merged['Actual RUL']
df_test_FD4['RUL']= df_FD4_merged["Actual Max Cycles"] - df_test_FD4["time, in cycles"].round(0).astype(int)
...
#df_test_last = df_test_FD4.merge(df_last_cycle_test, on=["Unit No.", "time, in cycles"], how = "inner")
# Select Features
df_train_FD4.to_excel("train_FD4_final.xlsx", index=True)

features = [col for col in df_train_FD4.columns if col not in ["RUL"]]
X_train = df_train_FD4[features]
y_train = df_train_FD4["RUL"]

X_test = df_test_FD4[features]
#X_test_last = df_test_last[features]
y_test = df_test_FD4["RUL"]
y_actual_rul = df_rul_FD4["Actual RUL"]
...
model = Sequential([
    LSTM(64, activation='tanh', return_sequences=True, kernel_regularizer=l2(0.01), input_shape=(X_train.shape[1], X_train.shape[2])),
    BatchNormalization(),
    Dropout(0.2),  # Reduced dropout

    LSTM(32, activation='tanh', kernel_regularizer=l2(0.01), return_sequences=True),
    BatchNormalization(),
    Dropout(0.2),

    LSTM(16, activation='tanh', return_sequences=False, kernel_regularizer=l2(0.01)),  # Smaller LSTM layer
    BatchNormalization(),
    Dropout(0.2),
...
X_val_features = feature_extractor.predict(X_val)  # Shape: (samples, features)
perm_importance = permutation_importance(model, X_val_features, y_val, scoring='neg_mean_absolute_error', n_repeats=10, random_state=42)

# Extract importance scores
feature_importance = perm_importance.importances_mean
sorted_idx = np.argsort(feature_importance)[::-1]
top_features = sorted_idx[:20]  # Get top 20 features

plt.figure(figsize=(12, 6))
plt.barh(range(20), feature_importance[top_features], align='center')
plt.yticks(range(20), X_train.columns[top_features])
