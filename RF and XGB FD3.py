...
df_test_last_FD3 = df_test_FD3.merge(df_last_cycle_test, on=["Unit No.", "time, in cycles"], how="inner")
...
y = df_train_FD3["RUL"]
X_test = df_test_last_FD3[features]
...
# Fit on training data (replace X_train, y_train with actual data)
...
df_results = pd.DataFrame({"Unit No.": df_test_last_FD3["Unit No."],"Predicted_RUL": y_test_pred})
df_results = df_results.merge(df_rul_FD3, on="Unit No.")
...
Optimized RMSE: 71.60
Optimized R² Score: 0.4953
Tested RMSE: 29.19
Tested R² Score: 0.5028
