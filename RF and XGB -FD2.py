...
features = [col for col in df_train_FD2.columns if col not in ["RUL","Unit No."]]
X = df_train_FD2[features]
y = df_train_FD2["RUL"]
X_test = df_test_last_FD2[features]
...
df_rul_FD2["Unit No."] = df_test_last_FD2["Unit No."].unique()
df_results = pd.DataFrame({"Unit No.": df_test_last_FD2["Unit No."],"Predicted_RUL": y_test_pred})
df_results = df_results.merge(df_rul_FD2, on="Unit No.")
...
'''
#Model FD2.py XGB score Optimized RMSE: 46.90
Optimized R² Score: 0.6141
Tested RMSE: 42.01
Tested R² Score: 0.3897
