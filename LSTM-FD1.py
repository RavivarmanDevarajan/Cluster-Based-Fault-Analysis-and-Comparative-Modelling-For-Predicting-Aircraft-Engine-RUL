...
df_rul_FD1 = pd.read_excel("C:/Users/raviv/.spyder-py3/CMAPSSData/RUL-01.xlsx")

# Drop NaN columns
df_train_FD1.dropna(axis=1, inplace=True)
df_test_FD1.dropna(axis=1, inplace=True)
...
max_cycles_test = df_test_FD1.groupby('Unit No.')['time, in cycles'].transform('max')
max_cycles_per_unit= max_cycles_test.to_frame()
max_cycles_per_unit["Unit No."]=df_test_FD1["Unit No."]
df_FD1_merged = max_cycles_per_unit.merge(df_rul_FD1, on='Unit No.', how='left')
df_FD1_merged['Actual Max Cycles'] = df_FD1_merged['time, in cycles'] + df_FD1_merged['Actual RUL']
df_test_FD1['RUL']= df_FD1_merged["Actual Max Cycles"] - df_test_FD1["time, in cycles"].round(0).astype(int)
...
X_train = df_train_FD1[features]
y_train = df_train_FD1["RUL"]

X_test = df_test_FD1[features]
#X_test_last = df_test_last[features]
y_test = df_test_FD1["RUL"]
...
model = Sequential([
    LSTM(128, activation='tanh', return_sequences=True, kernel_regularizer=l2(0.001),input_shape=(X_train.shape[1], X_train.shape[2])),
    BatchNormalization(),
    Dropout(0.3),

    LSTM(64, activation='tanh',kernel_regularizer=l2(0.001),return_sequences=True),
    BatchNormalization(),
    Dropout(0.3),

    LSTM(32, activation='tanh', return_sequences=False,kernel_regularizer=l2(0.001)),  # Last LSTM layer
    BatchNormalization(),
    Dropout(0.3),
